...
async def register(body: RegisterRequest, request: Request, db: Session = Depends(get_db)):
    ...
    if password_error:
        ...
        logger.info("REGISTER_REJECTED email=%s reason=weak_password", body.email)
...
async def login(body: LoginRequest, request: Request, db: Session = Depends(get_db)):
    ...
    if AuthService.is_user_locked(user):
        ...
        logger.warning("LOGIN_BLOCKED user_id=%s reason=locked", user.id)
        raise HTTPException(
    ...
    if not AuthService.verify_password(body.password, user.hashed_password):
        ...
        logger.warning(
            "LOGIN_FAILED user_id=%s reason=%s",
            user.id,
            "locked_threshold" if locked else "bad_password",
        )
